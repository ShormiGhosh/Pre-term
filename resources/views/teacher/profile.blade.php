    ...
    <div style="background: #1c1a36; border-radius: 12px; padding: 3rem; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);">
        ...
        <div style="display: flex; align-items: center; gap: 3rem; margin-bottom: 3rem; padding-bottom: 2rem; border-bottom: 2px solid #302e4a;">
            ...
            <div style="flex-shrink: 0;">
                @if($teacher->profile_image)
                    <img src="{{ asset('uploads/profiles/' . $teacher->profile_image) }}" 
                         alt="Profile Image" 
            ...
            <!-- Name and Role -->
            <div style="flex: 1;">
                <h1 style="color: #F1F5FB; font-size: 2.5rem; margin: 0 0 0.5rem 0; font-weight: 600;">{{ $teacher->name }}</h1>
                <p style="color: #C1CEE5; font-size: 1.2rem; margin: 0;">Teacher</p>
            </div>
            ...
            <div style="display: flex; gap: 1rem; align-items: center;">
                ...
                <form action="{{ route('teacher.profile.delete') }}" method="POST" 
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 2rem;">
            ...
            <div style="background: #302e4a; padding: 1.5rem; border-radius: 8px;">
                ...
                <p style="color: #F1F5FB; font-size: 1.1rem; margin: 0; word-break: break-word;">{{ $teacher->email }}</p>
            </div>

            <!-- Designation -->
            <div style="background: #302e4a; padding: 1.5rem; border-radius: 8px;">
                <p style="color: #C1CEE5; font-size: 0.9rem; margin: 0 0 0.5rem 0; text-transform: uppercase; letter-spacing: 0.5px;">Designation</p>
                <p style="color: #F1F5FB; font-size: 1.1rem; margin: 0;">{{ $teacher->designation }}</p>
            </div>

            <!-- Department -->
            <div style="background: #302e4a; padding: 1.5rem; border-radius: 8px; grid-column: span 2;">
                <p style="color: #C1CEE5; font-size: 0.9rem; margin: 0 0 0.5rem 0; text-transform: uppercase; letter-spacing: 0.5px;">Department</p>
                <p style="color: #F1F5FB; font-size: 1.1rem; margin: 0;">{{ $teacher->department }}</p>
            </div>
